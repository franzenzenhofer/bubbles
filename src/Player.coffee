...
class Player extends CircleMovingInGameObject
  ...
  update: =>
    ...
    if keydown.left then @x_velocity = @x_velocity - DEFAULT_USER_ACCELERATION
    #if keydown.right then @cx += 5
    if keydown.right then @x_velocity = @x_velocity + DEFAULT_USER_ACCELERATION
    #if keydown.up then @cy -=5
    if keydown.up then @y_velocity = @y_velocity - DEFAULT_USER_ACCELERATION
    #if keydown.down then @cy += 5
    if keydown.down then @y_velocity = @y_velocity + DEFAULT_USER_ACCELERATION
